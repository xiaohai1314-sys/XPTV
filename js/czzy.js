...
async function getCards(ext) {
    ...
    try {
        ...
        // 💡 [修正] 关键：设置 pagecount 为 1，阻止首页分类的无限加载
        return jsonify({ 
            list: cards,
            page: 1,
            pagecount: 1,
...
async function search(ext) {
    ...
    try {
        ...
        const pageCount = response.data?.data?.pagecount || (cards.length > 0 ? pageNum + 1 : pageNum); // 简单估算
        const totalCount = response.data?.data?.total || 9999; // 假设总数

        log(`[search] ✓ API成功返回并格式化 ${cards.length} 个卡片. Page: ${pageNum}/${pageCount}`);
...
// 💡 [修正] category 函数会传递页码 (pg)，getCards 现在会忽略它，但函数签名保持不变
