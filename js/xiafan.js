/**
 * HDHive 影视资料库 - App插件脚本 (终极认证版 V4.0)
 * 
 * 版本说明:
 * - 【终极修正】拨乱反正！废除所有关于“公开API”的错误判断，回归到“所有API请求均需认证”的正确策略，根治页面空白问题。
 * - 【强制认证】重写 fetchApi 函数，确保每一次对API的请求都携带完整的、正确的Cookie和认证头，完全模拟真实浏览器的行为。
 * - 【解密核心】内置Pako.js解压库，用于正确解析分类页返回的加密数据。
 * - 【精准API】所有功能（分类、详情、搜索）均使用我们已验证的、正确的API端点和请求方式。
...
// --- 核心辅助函数 ---
function log(msg ) { 
    try { $log(`[HDHive 插件 V4.0] ${msg}`); } 
    catch (_) { console.log(`[HDHive 插件 V4.0] ${msg}`); } 
}
