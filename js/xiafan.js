/**
 * HDHive 影视资料库 - App插件脚本 (最终解密版 V3.0)
 * 
 * 版本说明:
 * - 【解密核心】内置Pako.js解压库，成功破解并解析分类页返回的加密数据，根治页面空白问题。
 * - 【混合API策略】智能区分需要认证的内部API（详情、搜索）和无需认证的公开API（分类），请求成功率最大化。
 * - 【精准筛选】详情页严格按照您的要求，仅提取和展示 115、阿里云盘、天翼云盘、夸克网盘 四种资源。
 * - 【缓存优化】完美集成了高级搜索缓存机制，切换关键词自动刷新，重复搜索和翻页实现秒开，体验流畅。
 * - 【配置核心】请务必在下方的【用户配置区】填入您自己的有效Cookie，以访问详情和搜索功能。
 */
...
// --- 网络请求 ---
async function fetchApi(method, url, params = {}, body = null, additionalHeaders = {}, needsAuth = true) {
    if (needsAuth && (!COOKIE || COOKIE.includes("YOUR_COOKIE_HERE"))) {
        $utils.toastError("请先在插件脚本中配置Cookie", 3000);
        throw new Error("Cookie not configured for an authenticated request.");
    }
    ...
    const headers = { 'User-Agent': UA, 'Content-Type': 'application/json', ...additionalHeaders };
    if (needsAuth) {
    ...
    log(`请求API: ${method} ${finalUrl.toString()} (认证: ${needsAuth})`);
...
async function getCards(ext) {
  ...
  try {
    const jsonData = await fetchApi('GET', `${API_BASE_URL}/public/${type}s`, { per_page: 24, share_num_gt: 0, page: page }, {}, false);
...
async function getTracks(ext) {
    ...
    try {
        const mainData = await fetchApi('GET', `${API_BASE_URL}/${type}/${slug}`, {}, {}, true);
        ...
        const rawResourceData = await fetchApi('POST', `${SITE_URL}/${type}/${slug}`, {}, resourceBody, resourceHeaders, true);
...
async function search(ext) {
  ...
  try {
    ...
    const jsonData = await fetchApi('POST', `${API_BASE_URL}/search`, {}, requestBody, true);
