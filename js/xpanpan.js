 ...
 * 
 * 整合所有优化点:
 * 1. [已修复] getTracks 函数的链接解析逻辑，确保所有链接都能显示。
 * 2. [已修复] play 函数增加对URL的“极致净化”逻辑，确保传递给App的链接100%纯净，解决“分享不存在”问题。
 * 3. [已保留] 完整的用户交互提示和剪贴板功能。
 * 4. [已保留] 对新旧版 XPTV App 接口的完全兼容。
 */

// --- 配置区 ---
const API_BASE_URL = 'http://192.168.1.6:3000/api'; // 请确保此地址对于运行 App 的设备是可访问的
// --- 配置区 ---
...
async function play(flag, id, ext) {
    ...
    for (const pattern of patterns) {
        ...
        if (match && match[1]) {
            pureUrl = match[1];
            log(`净化成功，最终URL: ${pureUrl}`);
            break;
        }
    }

    if (!pureUrl) {
        const fallbackMatch = id.match(/https?:\/\/[^\s]+/);
        pureUrl = fallbackMatch ? fallbackMatch[0] : '';
        log(`规则匹配失败，使用备用逻辑提取URL: ${pureUrl}`);
    }

    if (!pureUrl) {
        log(`错误: 无法从ID[${id}]中提取有效URL`);
