...
const SITE = 'https://www.wpzysq.com'; // 添加网站根URL
// --- 配置区 ---
...
async function getCards(ext) {
  ...
  const cards = (data.list || []).map(item => ({
    ...
    vod_pic: item.vod_pic ? (item.vod_pic.startsWith('http') ? item.vod_pic : `${SITE}/${item.vod_pic}`) : '', // 确保图片是完整URL
...
async function getTracks(ext) {
  ...
  if (data.list && data.list.length > 0) {
    ...
    // 添加海报图片
    if (detailItem.vod_pic) {
      tracks.push({
        name: '海报',
        pan: detailItem.vod_pic.startsWith('http') ? detailItem.vod_pic : `${SITE}/${detailItem.vod_pic}`,
        ext: {},
      });
    }
    // 添加详情内容
    if (detailItem.vod_content) {
      tracks.push({
        name: '详情',
        pan: detailItem.vod_content,
        ext: {},
      });
    }
    // 添加网盘链接
