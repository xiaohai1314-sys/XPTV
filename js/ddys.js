...
async function detail(id) {
    ...
    try {
        ...
        // ★★★★★ 关键修复：完美兼容返回字符串的各种奇葩TVBox分支 ★★★★★
        let realData = data;
        if (typeof realData === 'string') {
            realData = realData.trim();
            if (realData.startsWith('\ufeff')) realData = realData.substring(1);
            try { realData = JSON.parse(realData); } catch(e) { realData = {}; }
        }
        
        if (!realData || !realData['115'] || !Array.isArray(realData['115'])) {
            log(`[detail] 115资源为空或解析失败，原始返回: ${JSON.stringify(realData)}`);
            return jsonify({ list: [] });
        }

        const tracks = realData['115'].map(item => ({
...
// ★★★★★ 终极兼容版 fetchData（只改了这一个函数，其他全原封不动）★★★★★
    ...
    try {
        ...
        if (typeof data === 'string') {
            ...
            try {
                ...
                log(`[fetchData] JSON解析失败，原始内容前200字符: ${data.substring(0,200)}...`);
        ...
        log(`[fetchData] 请求彻底失败: ${err.message}`);
        throw err;
    }
}
