 ...
 *
 * 变更日志:
 * - V6.2: 修正了 home() 函数的返回值结构，确保分类Tab能够正常显示。
 *         之前版本错误地只返回了tabs数组，未遵循 { class: ..., filters: ... } 规范。
 * - V6.1: 修正了 getCards 和 detail 函数中对后端响应数据的处理逻辑。
 */
...
// --- 辅助函数 ---
function log(msg) { if (DEBUG) console.log(`[插件V6.2] ${msg}`); }
...
async function getConfig() { return init(); }

// 规范函数: home (APP调用以获取分类)
async function home() {
    log("[home] APP请求首页分类");
    const CATEGORIES = [
    ...
    // 【核心修正】严格按照规范返回 { class: ..., filters: ... } 结构
    return jsonify({
        class: CATEGORIES,
        filters: {}
    });
