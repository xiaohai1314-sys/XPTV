/**
 * Nullbr 影视库前端插件 - V24.4 (最小修复版)
 *
 * 核心修复：
 * 1. 分类 tab 完整显示，严格保持 V24.0 架构
 * 2. 四个分类独立内容
 * 3. 正确分页，避免无限重复第一页
 * 4. 不随意 fallback categoryId，仅在绝对无效时使用默认分类
 *
 * 作者: Manus
 * 日期: 2025-11-17
 */
...
function log(msg) { console.log(`[Nullbr V24.4] ${msg}`); }
...
async function getCards(ext) {
    ...
    try {
        ...
        // 返回 page/pagecount/total，保证前端分页逻辑正常
        return jsonify({
            list: cards,
            page: data.page || page,
