/**
 * Nullbr 影视库前端插件 - V40.3 (终极集成 + 异步修正版)
 *
 * 目标:
 * 1. 【Tab 修正】将 init/getConfig/home 改回 async function，确保分类 Tab 正常显示 (解决 V40.2 失败的问题)。
 * 2. 【通信修复】使用路径参数 /api/list/ID?page=... 结构 (配合后端V3.0)。
 * 3. 【运行时修复】category 函数内部使用最原始的 ES3 语法，同时支持双重名称查找 (解决分类 ID 错误的问题)。
 *
...
function log(msg) { console.log("[Nullbr V40.3] " + msg); } 
...
// ---------------- 入口：init / getConfig / home (已修正为异步) ----------------

async function init(ext) { return getConfig(); } // 改回 async

async function getConfig() { // 改回 async
    return jsonify({
        ver: 40.3,
        title: 'Nullbr影视库 (V40.3)', // 标题更新
        site: API_BASE_URL,
        tabs: CATEGORIES
    });
}

async function home() { // 改回 async
...
async function search(wd, quick) { return jsonify({ url: "" }); }
