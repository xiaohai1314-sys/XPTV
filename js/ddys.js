/**
 * Nullbr 影视库前端插件 - V40.1 (Zero-Tolerance ES3 + 路径参数通信版)
 *
 * 目标:
 * 1. 【通信修复】使用路径参数 /api/list/ID?page=... 结构 (配合后端V3.0)。
 * 2. 【运行时修复】使用最原始的 ES3 语法，同时支持双重名称查找 (防止 category() 崩溃)。
 *
...
function log(msg) { console.log("[Nullbr V40.1] " + msg); } 
...
async function getCards(ext) {
    ...
    if (!categoryId) {
        // Fallback should be rare here, handled in category()
