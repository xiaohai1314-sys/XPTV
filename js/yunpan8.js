/**
 * 海绵小站前端插件 - v9.4.1 (最终版 - 错误处理修正)
 *
 * 更新说明:
 * - 基于被验证可行的 v9.4 版本。
 * - 完全保留用户原始的、正确的链接和密码提取逻辑。
 * - 仅修正当无法连接后端时，错误提示显示为 "undefined" 的小问题。
 * - 这是最稳定、最忠于原始逻辑的版本。
 */
...
function log(msg  ) { try { $log(`[海绵小站 v9.4.1] ${msg}`); } catch (_) { console.log(`[海绵小站 v9.4.1] ${msg}`); } }
