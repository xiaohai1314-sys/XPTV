 ...
 * 
 * 版本说明:
 * - v8.0: 【终极净化】增加了对链接和访问码的“双重过滤”净化逻辑，彻底清除所有不可见的“幽灵字符”和尾随垃圾，解决特定帖子无法识别的问题。
 * - 继承了之前版本的所有修复和功能。
 */
    ...
    try {
        ...
        if (uniqueLinks.length > 0) {
            uniqueLinks.forEach((link, index) => {
                ...
                // 1. 净化链接: 砍掉所有尾随的非字母数字字符
                const originalLink = link;
                while (/[^a-zA-Z0-9]$/.test(link)) {
                    link = link.slice(0, -1);
                }
                if (originalLink !== link) {
                    log(`链接净化: "${originalLink}" -> "${link}"`);
                }

                // 2. 净化访问码: 只保留可见的ASCII字符，清除所有“幽灵”
                let code = codePool[index] || '';
                const originalCode = code;
                code = code.replace(/[^\x20-\x7E]/g, '');
                if (originalCode !== code) {
                    log(`访问码净化: "${originalCode}" -> "${code}"`);
                }
                // ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
