/**
 * 海绵小站前端插件 - 融合修正版 (V10.0)
 * 
 * 版本说明:
 * - 【v10.0 融合版】根据用户反馈的“V7能用，V9不能用”问题进行深度分析和修正。
 * - 核心修改：在访问码提取的最后一步，采用“双层兜底”策略。
 *   1. 优先使用V7.0版本的 `match(/^[a-zA-Z0-9]+/)` 逻辑，因为它已被验证在您的环境中可用。
 *   2. 如果V7.0的逻辑提取失败（返回null），则自动启用V9.0的 `match(/[a-zA-Z0-9]/g)` 全局重组逻辑作为第二道保险。
 * - 此版本旨在100%兼容V7.0可用的场景，同时增强对其他复杂格式的识别能力。
 * - 其他所有部分，包括拼接格式、数据结构等，均与V7.0保持一致。
 */
    ...
    try {
        ...
        if (codePool.length === 0) {
            ...
            while ((match = fallbackRegex.exec(mainMessageText)) !== null) {
                ...
                // 1. 优先尝试V7.0的“从头匹配”逻辑
                const v7Match = convertedCode.match(/^[a-zA-Z0-9]+/);
                if (v7Match) {
                    finalCode = v7Match[0];
                    log(`V7.0逻辑提取成功: "${finalCode}"`);
                } else {
                    log("V7.0逻辑提取失败，启用V9.0的全局重组逻辑...");
                    // 2. 如果V7.0失败，则启用V9.0的“全局重组”逻辑
                    const v9Match = convertedCode.match(/[a-zA-Z0-9]/g);
                    if (v9Match && v9Match.length > 0) {
                        finalCode = v9Match.join('');
                        log(`V9.0逻辑提取成功: "${finalCode}"`);
                    } else {
