// ==UserScript==
// @name         海绵小站插件 - v62.1-final-fix
// @version      62.1
// @description  修复部分帖子嵌套<a>导致按钮名称显示异常的问题（如黑鹰坠落）
// ==/UserScript==
...
function log(msg) { try { $log(`[海绵小站 V62.1-fix] ${msg}`); } catch (_) { console.log(`[海绵小站 V62.1-fix] ${msg}`); } }
    ...
    try {
            ...
            if (replied) {
                ...
                return jsonify({
                    list: [
                        {
                            title: '提示',
                            tracks: [{ name: "Cookie无效或未配置，无法获取资源", pan: '', ext: {} }]
                        }
                    ]
                });
            }
        ...
        if (uniqueLinks.length > 0) {
            uniqueLinks.forEach((link, index) => {
                ...
                const finalPan = code ? `${link}（访问码：${code}）` : link;
        ...
        return jsonify({
            list: [{ title: '错误', tracks: [{ name: "操作失败，请检查Cookie配置和网络", pan: '', ext: {} }] }]
        });
    }
}
...
log('海绵小站插件加载完成 (v62.1-final-fix)');
