/**
 * 海绵小站前端插件 - 移植增强版 (基于最终修复版 + V9 提取逻辑)
 * 
 * 更新说明:
 * - 移植 V9 的防误判访问码提取逻辑
 * - 支持天翼云盘 (cloud.189.cn) 与 夸克网盘 (pan.quark.cn)
 * - 关键词提取 + 纯净码兜底，避免误判 URL
 * - 使用 Map 去重，优先保留带访问码的记录
 * - 保留原有接口结构，确保在 APP 内可直接运行
 */
...
function log(msg) { try { $log(`[海绵小站 移植增强版] ${msg}`); } catch (_) { console.log(`[海绵小站 移植增强版] ${msg}`); } }
...
async function search(ext) {
  ...
  if (searchCache.keyword !== text) { searchCache.keyword = text; searchCache.data = []; searchCache.pagecount = 0; searchCache.total = 0; }
  if (searchCache.data[page - 1]) return jsonify({ list: searchCache.data[page - 1], pagecount: searchCache.pagecount, total: searchCache.total });
  if (searchCache.pagecount > 0 && page > searchCache.pagecount) return jsonify({ list: [], pagecount: searchCache.pagecount, total: searchCache.total });

  let url = page === 1 ? `${SITE_URL}/search.htm?keyword=${encodeURIComponent(text)}` : `${SITE_URL}/search-${encodeURIComponent(text)}-1-0-${page}.htm`;
  try {
    const { data } = await fetchWithCookie(url);
    const $ = cheerio.load(data);
    const cards = [];
    ...
    paginationLinks.each((_, link) => { const p = parseInt($(link).text().trim()); if (!isNaN(p)) pagecount = Math.max(pagecount, p); });
    const total = cards.length;
    searchCache.data[page - 1] = cards; searchCache.pagecount = pagecount; searchCache.total = total;
