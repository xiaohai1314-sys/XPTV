/**
 * 七味网(qwmkv.com) - 纯网盘提取脚本 - v4.1 (修复版)
 *
 * 版本说明:
 ...
 *
 * v4.1 更新日志:
 * - [修复] 修正了 getCards 函数中的数据处理逻辑。之前由于不正确的解构赋值，
 *   导致从后端获取到数据后，前端无法正确解析 list 数组，从而出现列表空白的问题。
 *   当前版本直接使用后端返回的完整数据，解决了该问题。
 */
...
async function getCards(ext) {
    ...
    try {
        // 【【【 已修复 】】】
        // 直接获取后端返回的完整响应数据 (它本身就是 { list: [...] } 结构)
        // 不再使用 const { data } = ... 进行错误的解构
        const responseData = await $fetch.get(url);
        log(`✅ 成功从后端获取到数据，即将渲染列表。`);
