...
async function search(ext) {
  ...
  $('a[href^="/s/"]').each((_, each) => {
    ...
    if (path.startsWith('/s/')) {
      const title = $(each).find('div').first().text().trim()   // 修复标题提取
      const pic = $(each).find('van-card').attr('thumb') || '/img/folder.png' // 修复图片提取
      cards.push({
        vod_id: path,
        vod_name: title || '未命名资源',
        vod_pic: appConfig.site + pic,
        vod_remarks: '',
        ext: {
